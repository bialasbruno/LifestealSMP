...
TARGET="${1:-all}"
case "$TARGET" in
  all|core|scoreboard|souls|soulitems|soulshop|spawn|homes) ;;
  -h|--help)
    usage
    exit 0
    ;;
...
case "$TARGET" in
  ...
  spawn)
    ...
    ;;
  homes)
    GRADLE_TASKS=(:LifestealHomes:clean :LifestealHomes:build)
    EXPECTED_JARS=("LifestealHomes/build/libs/LifestealHomes-0.1.0.jar")
    ;;
esac
